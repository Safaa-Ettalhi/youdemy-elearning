...
class VideoCourse extends Course {
    public function __construct($pdo, $titre = '', $description = '', $contenu = '', $image = null, $prix = 0, $enseignant_id = null, $categorie_id = null) {
        parent::__construct($pdo, null, $titre, $description, $contenu, $image, $prix, 'video', $enseignant_id, $categorie_id);
    }
    public function addCourse($title, $description, $filePath, $imagePath, $prix, $teacherId, $categoryId, $tags, $contentType) {
        $sql = "INSERT INTO cours (titre, description, vedio, image, prix, user_id, categorie_id, typeContenu)
    ...
    $stmt = $this->pdo->prepare("
        SELECT Cours.*, 
            ...
            COUNT(DISTINCT Enrollment.user_id) AS nbr_etudiants
        FROM Cours
    ...
    $stmt = $this->pdo->prepare("
        ...
        LEFT JOIN Utilisateur ON Cours.user_id = Utilisateur.id
        LEFT JOIN Enrollment ON Cours.id = Enrollment.cours_id
        WHERE Cours.id = :id AND Cours.typeContenu = 'video'
...
public function getCoursesByEnseignant($enseignant_id) {
        ...
        $stmt = $this->pdo->prepare("SELECT c.*, cat.nom AS categorie, 
         ...
         FROM Cours c
         LEFT JOIN Category cat ON c.categorie_id = cat.id WHERE user_id = :enseignant_id AND typeContenu = 'video'");
